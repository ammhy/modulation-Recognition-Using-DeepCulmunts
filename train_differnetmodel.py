...
# 加载配置文件
with open('model_lr.json', 'r') as f:
    config = json.load(f)
...
for model_config in config['models']:
    ...
    # 动态导入模型
    if model_name == "ResNet18LSTMTransformer":
        from model import ResNet18LSTMTransformer
        model=ResNet18LSTMTransformer(num_classes=num_classes, input_size=input_dim)
    elif model_name == "ResNet18WithTransformer":
        from model_resnet_transformer import ResNet18WithTransformer
        model = ResNet18WithTransformer(num_classes=num_classes)
    elif model_name == "fft_transformer_cumulants":
        from model_fft_transformer import fft_transformer_cumulants
        model = fft_transformer_cumulants(num_classes=num_classes)
    elif model_name == "cqtAndAttentioncomplex":
        from model_cqt_complex import  EnhancedResNet34
        model =  EnhancedResNet34(num_classes=num_classes)
    elif model_name == "cqtAndAttention":
        from model_cqt_attention import  ResNet34WithEnhancedFourier
        model =  ResNet34WithEnhancedFourier(num_classes=num_classes)
    else:
    ...
    best_accuracy = 0.0
    early_stopper = EarlyStopper(patience=30, min_delta=0.001, mode='min')
    ...
    for epoch in range(num_epochs):
        ...
        if accuracy > best_accuracy:
            ...
            torch.save(model.state_dict(), f'nine_signal/train/{model_name}.pth')
            logging.info('\nBest model saved.')
    ...
    # 最终测试（只在训练完成后执行一次）
    model.load_state_dict(torch.load(f'nine_signal/train/{model_name}.pth'))
    model.eval()
    test_correct = 0
    with torch.no_grad():
